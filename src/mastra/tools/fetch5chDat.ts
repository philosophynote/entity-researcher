...
function parse5chUrl(url: string): { host: string, board: string, key: string } | null {
  // itest.5ch.net/greta/test/read.cgi/poverty/1746103545/
  // asahi.5ch.net/test/read.cgi/newsplus/1747116664
  const m = url.match(/^https?:\/\/(?:(itest|(\w+))\.5ch\.net)\/(\w+)\/test\/read\.cgi\/(\w+)\/(\d+)/)
...
export const fetch5chDat = createTool({
  id: 'fetch5chDat',
  description: '5chのスレッドURLを受け取り、datスレッドを取得し各レスを配列で返します',
  inputSchema: z.object({
    url: z.string().url().describe('5chスレッドのURL（例: https://itest.5ch.net/greta/test/read.cgi/poverty/1746103545/）'),
  }),
  ...
  execute: async ({ context: { url } }) => {
    console.log('[fetch5chDat] 入力URL:', url);
    ...
    console.log('[fetch5chDat] パース結果:', { host, board, key });
    const datUrl = `https://${host}/${board}/dat/${key}.dat`;
    console.log('[fetch5chDat] dat取得URL:', datUrl);
    const res = await fetch(datUrl, {
      headers: {
        // Monazilla は 5ch が公式に認めている UA
        'User-Agent': 'Monazilla/1.00 (ChatGPT Example)',
        // gzip 転送を受け取る場合もあるので Accept-Encoding は省略が安全
      },
    });
    if (!res.ok) {
    ...
    console.log('[fetch5chDat] dat取得・デコード成功');

    // 各レスをオブジェクト化
    const posts = txt.trim().split('\n').map((line, i) => {
    ...
    console.log(`[fetch5chDat] パース完了: ${posts.length}件`);
...
export const searchCompanyInPosts = createTool({
  ...
  description: 'posts配列と企業名を受け取り、bodyに企業名が含まれる投稿または類似投稿をベクトル検索で返します',
  inputSchema: z.object({
    posts: z.array(z.object({
      no: z.number(),
      name: z.string(),
  ...
  execute: async ({ context: { posts, companyName } }) => {
    ...
    const bodyEmbeddingRes = await embeddingModel.doEmbed({ values: bodyTexts });
    const bodyEmbeddings = bodyEmbeddingRes.embeddings;

    // クエリ（企業名）の埋め込み
    const queryEmbeddingRes = await embeddingModel.doEmbed({ values: [companyName] });
