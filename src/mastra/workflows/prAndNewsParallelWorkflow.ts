...
export const classifyRiskStep = createStep({
  ...
  execute: async ({ inputData }) => {
    const results:
      | { title: string; url: string; label: string; reason: string }[]
      | PromiseLike<
          { title: string; url: string; label: string; reason: string }[]
        > = [];
...
export const makeCorporateNumberPrompt = createStep({
  ...
  description: "法人番号を企業情報収集エージェント向けに渡す",
...
const initialStep = createStep({
  ...
  async execute({ inputData }) {
    return {
      ...
      corporateNumber: inputData.corporateNumber,
...
export const socialInsuranceFlow = createWorkflow({
  ...
  .map({
    corporateNumber: {
      step: makeCorporateNumberPrompt,
...
export const prAndNewsParallelWorkflow = createWorkflow({
  ...
  .map({
    ...
    keyword: {
      step: initialStep,
      path: "companyName",
      schema: z.string().describe("検索キーワード"),
    },
    companyName: {
      step: initialStep,
      path: "companyName",
      schema: z.string().describe("企業名"),
    },
    corporateNumber: {
      step: initialStep,
      path: "corporateNumber",
      schema: z.string().length(13).describe("法人番号"),
    },
    address: {
      step: initialStep,
      path: "address",
      schema: z.string().describe("住所"),
    },
  })
